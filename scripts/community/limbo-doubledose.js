strategyTitle = 'DoubleDose (Limbo)';
author = 'captde.v';
version = '1.2';
...
initialChance = 98; // internal win chance — drives target calculation: target = 99 / chance
...
================================
Scripted by ${scripter} for Antebot Originals (Limbo)
-------------------------------------------
`);
}
...
function execute(doAction) {
    switch (doAction.type) {
        ...
        case 'switchOverUnder':
            // no-op in limbo (no over/under concept)
            break;
        case 'resetAmount':
            betSize = initialBetSize;
            break;
...
Wagered: ${this.wagered.toFixed(8)} / ${this.wagered.toFiatString()} (${(this.wagered / startBalance * 100).toFixed(2)}%)`);
        for (var ri = 0; ri < resetLog.length; ri++) {
            log('#FFFF2A', resetLog[ri]);
        }
    }
};
...
engine.onBetPlaced((lastBet) => {
    ...
    if ((resetOnProfit > 0 && stats.profit >= resetOnProfit) || (resetOnLoss > 0 && -stats.profit >= resetOnLoss)) {
        ...
        resetLog.push('Cycle #' + (resetLog.length + 1) + ' (' + resetReason + ' ' + currency + ')');
        betSize = initialBetSize;
        chance = initialChance;
        target = 99 / chance;
        stats.resetStats();
